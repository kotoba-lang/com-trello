(ns trello.main
  "Kotodama WASM entrypoint for the Trello clean-room actor (L4) — Clojure port.
  ...
  facts (`trello.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "trello")
(def tier "L4")
...
(def entity-specs
  [{:entity "Card"   :plural "cards"   :id-prefix "trello_car"
    :fields [:idShort :name :desc :closed :due :dueComplete :pos :url :idBoard :idList :cardRole :dateLastActivity] :required [:idShort]
    :coerce {:idShort :int :pos :float :closed :bool :dueComplete :bool}          :refs {}}
   {:entity "Board"  :plural "boards"  :id-prefix "trello_boa"
    :fields [:name :desc :closed :url :idOrganization :pinned :starred :dateLastActivity] :required [:name]
    :coerce {:closed :bool :pinned :bool :starred :bool}          :refs {}}
   {:entity "List"   :plural "lists"   :id-prefix "trello_lis"
    :fields [:name :closed :pos :idBoard]                          :required [:name]
    :coerce {:closed :bool :pos :float}                 :refs {}}
   {:entity "Member" :plural "members" :id-prefix "trello_mem"
    :fields [:username :fullName :initials :memberType :confirmed] :required [:username]
    :coerce {:confirmed :bool}                           :refs {}}
   {:entity "Label"  :plural "labels"  :id-prefix "trello_lab"
    :fields [:idBoard :name :color]                      :required [:idBoard]
    :coerce {}                                :refs {}}])
...
(defn emit-facts
  "EAVT facts for one record: {\"trello.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "trello-compat" :tier tier :entities entities} 200])
...
(defn start! [] :trello-compat/ready)
